"""
M2 v2 - Canal optimo para Mibanco-confIA (con balanceo de clases).
...
Mejora v2: compute_sample_weight("balanced") para compensar desbalance campo(6%) y sms(22%).
"""
...
def main():
    print("M2 v2 — Cargando contactos con pago=1 ...")
    ...
    device = "cpu"
    print(f"  Entrenando XGBoost M2 v2 [{device}] con balanceo de clases ...")

    # Balanceo de clases: campo(6%) y sms(22%) aprenden mejor
    from sklearn.utils.class_weight import compute_sample_weight
    sw_tr = compute_sample_weight("balanced", ytr)
    sw_va = compute_sample_weight("balanced", yva)
    ...
    xgbm.fit(Xtr, ytr, sample_weight=sw_tr, eval_set=[(Xva, yva)],
             sample_weight_eval_set=[sw_va], verbose=False)
    best_it = xgbm.best_iteration

    pred  = xgbm.predict(Xte)
    proba = xgbm.predict_proba(Xte)
    acc   = accuracy_score(yte, pred)
    print(f"  M2 v2 ({best_it} arb.) | Accuracy test: {acc:.1%}\n")
    ...
    meta = {
        ...
        "descripcion": "Canal optimo v2: balanceo de clases (campo 6%, sms 22%) sobre contactos exitosos (pago=1)",
    }
